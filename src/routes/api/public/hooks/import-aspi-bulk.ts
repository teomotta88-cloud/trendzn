...
// Import in bulk dei canali per la pagina ASPI-monitoring da un file Excel/CSV
// con due colonne (Nome canale, URL). Scrive TUTTI i canali nuovi in un solo
// commit su aspi-monitoring.json (chiave "canali"), con retry sullo sha per
...
export const Route = createFileRoute("/api/public/hooks/import-aspi-bulk")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            ...
            const writeRes = await fetch(
              `https://api.github.com/repos/${GITHUB_REPO}/contents/${ASPI_PATH}`,
              {
                ...
                body: JSON.stringify({
                  message: `chore: import bulk ${added} canali ASPI [trendzn-manual]`,
