...
import { aspiCanali } from "@/lib/aspiMonitoring";
import { SocialEmbed, PlatformIcon } from "@/components/SocialEmbed";
import { ArrowLeft, ExternalLink, Search } from "lucide-react";

// Dettaglio canale ASPI-monitoring — copia di canali-inspo.$id.tsx, legge dallo
// store dedicato (aspiCanali, bundlato da aspi-monitoring.json).

export const Route = createFileRoute("/aspi-monitoring/$id")({
  head: () => ({
    meta: [
      { title: "Canale — ASPI-monitoring" },
...
function Page() {
  ...
  const canale: CanaleInspo | undefined = aspiCanali.find((c) => String(c.id) === String(id));
  ...
  if (!canale) {
    return (
      <div className="py-20 text-center">
        ...
        <Link to="/aspi-monitoring" className="mt-4 inline-block text-primary">
          Torna ai canali
        </Link>
      </div>
    );
