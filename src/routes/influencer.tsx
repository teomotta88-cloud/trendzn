...
function InfluencerPage() {
  ...
  const dbProfiles = useMemo(
    () => dbRows.map(rowToProfile).filter((c) => !c.accounts.some((a) => jsonHandles.has(a.handle.toLowerCase()))),
    [dbRows, jsonHandles],
  );
          ...
          return (
            <div key={c.id} className="group relative">
              ...
              {(() => {
                ...
                if (isJsonProfile) {
                  return (
                    <Link to="/influencer/$id" params={{ id: c.id }} className={cardClassName}>
                      {cardContent}
                    </Link>
                  );
                }
